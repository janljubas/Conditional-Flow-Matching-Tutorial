...
def resolve_settings(cfg) -> dict:
    ...
    s["cfm_variant"] = str(s.get("cfm_variant", "icfm")).lower()
    ...
    if s["cfm_variant"] not in {"icfm", "otcfm"}:
        raise ValueError(f"Unsupported cfm_variant '{s['cfm_variant']}'. Choose 'icfm' or 'otcfm'.")
    if s["num_channels"] % 32 != 0:
        raise ValueError(f"num_channels must be divisible by 32 (torchcfm GroupNorm32), got {s['num_channels']}.")
